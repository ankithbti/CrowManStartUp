	...
	static class Layer {
		...
		Layer(int segmentIndex, float scrollRate) {
			...
			for (int i = 0; i < layers.length; ++i) {
				layers[i] = assetManager().getImage(
						"Backgrounds/Layer" + segmentIndex + "_" + i + ".png");
			}
		}
		...
		void Draw(Surface surf, float cameraPosition) {
			for (int i = 0; i <= EntityLayer; ++i) {
				...
				System.out.println("Seg width: " + segmentWidth);
				// Calculate which segments to draw and how much to offset them.
				float x = cameraPosition * scrollRate;
				int leftSegment = (int) Math.floor(x / segmentWidth);
				int rightSegment = leftSegment + 1;
				x = (x / segmentWidth - leftSegment) * -segmentWidth;
				System.out.println("Drawing Image: " + (leftSegment % layers.length) + " at x: " + x);
				System.out.println("Drawing Image: " + (rightSegment % layers.length) + " at x: " + (x + segmentWidth));
				surf.drawImage(layers[leftSegment % layers.length], x, 0);
				surf.drawImage(layers[rightSegment % layers.length], x
						+ segmentWidth, 0);
			}
		}
	}
...
//	    System.out.println("Old Camera Position: " + cameraPosition);
//	    System.out.println("Player Postion: " + Player().Position().X + " " + Player().Position().Y);
//	    System.out.println("MarginLeft: " + marginLeft + " MarginRight: " + marginRight);
//	    System.out.println("MarginWidth: " + marginWidth);
//	    System.out.println("Camera Movement: " + cameraMovement);
	    // Update the camera position, but prevent scrolling off the ends of the level.
	    float maxCameraPosition = Tile.Width * Width() - graphics().width();
//	    System.out.println("Max Camera Position: " + maxCameraPosition);
