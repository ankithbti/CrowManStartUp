...
public class CrowMan implements Game {
	...
	// When the time remaining is less than the warning time, it blinks on the
	// hud
	...
	// The number of levels in the Levels directory of our content. We assume
	// that
	// levels in our content are 0-based and that all numbers under this
	// constant
	// have a level file present. This allows us to not need to check for the
	// file
	...
	public void init() {
		...
		layer = graphics().createSurfaceLayer(graphics().width(),
				graphics().height());
	...
	public void update(float delta) {
		...
		// update our level, passing down the GameTime along with all of our
		// input
		// states
		level.Update(delta / 1000, keyboardState, gamePadState, touchState,
				accelerometerState, DisplayOrientation.LandscapeRight);
	}
	...
	private void HandleInput() {
		...
		boolean continuePressed = keyboardState.IsKeyDown(Key.SPACE)
	...
	private void DrawHud() {
		Rectangle titleSafeArea = new Rectangle(0, 0, graphics().width(),
				graphics().height());
		Vector2 hudLocation = new Vector2(titleSafeArea.Left, titleSafeArea.Top);
		Vector2 center = new Vector2(titleSafeArea.Left + titleSafeArea.Width
				/ 2.0f, titleSafeArea.Top + titleSafeArea.Height / 2.0f);

		// Draw time remaining. Uses modulo division to cause blinking when the
		// player is running out of time.
		String timeString = "TIME: "
				+ Integer.toString((int) (level.TimeRemaining() / 60)) + ":"
		...
		if (status != null) {
			...
			layer.surface().drawImage(status, center.X - status.width() / 2,
					center.Y - status.height() / 2);
		}
	}
